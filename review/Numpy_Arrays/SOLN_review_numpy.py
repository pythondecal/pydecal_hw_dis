...
0 to 2pi (you'll need to look up a function that does this). Then create 
a second array where each value in the array is double that of the previous 
array (call these `x1` and `x2` respectively)
"""
...
print(f"mean x1: {np.mean(x1)}")
print(f"median x1: {np.median(x1)}")
print(f"std x1: {np.std(x1)}")
print("\n") #add a new line
print(f"mean x2: {np.mean(x2)}")
print(f"median x2: {np.median(x2)}")
print(f"std x2: {np.std(x2)}")
...
avg_lc = np.mean(exoplanet_data, axis=0)
